// 20240208124228
// https://raw.githubusercontent.com/aeon0/d4lf/main/assets/lang/enUS/sigils.json

export const sigils = {
  dungeons: {
    vault_of_copper: "vault of copper",
    vault_of_cinder: "vault of cinder",
    vault_of_ink: "vault of ink",
    vault_of_stone: "vault of stone",
    anicas_claim: "anicas claim",
    black_asylum: "black asylum",
    caldera_gate: "caldera gate",
    cultist_refuge: "cultist refuge",
    dead_mans_dredge: "dead mans dredge",
    defiled_catacomb: "defiled catacomb",
    derelict_lodge: "derelict lodge",
    forbidden_city: "forbidden city",
    forsaken_quarry: "forsaken quarry",
    hallowed_ossuary: "hallowed ossuary",
    hoarfrost_demise: "hoarfrost demise",
    immortal_emanation: "immortal emanation",
    kor_dragan_barracks: "kor dragan barracks",
    kor_valar_ramparts: "kor valar ramparts",
    lights_watch: "lights watch",
    lost_archives: "lost archives",
    maulwood: "maulwood",
    mercys_reach: "mercys reach",
    nostrava_deepwood: "nostrava deepwood",
    rimescar_cavern: "rimescar cavern",
    tormented_ruins: "tormented ruins",
    sanguine_chapel: "sanguine chapel",
    zenith: "zenith",
    akkhans_grasp: "akkhans grasp",
    ancient_reservoir: "ancient reservoir",
    bastion_of_faith: "bastion of faith",
    belfry_zakara: "belfry zakara",
    blind_burrows: "blind burrows",
    earthen_wound: "earthen wound",
    endless_gates: "endless gates",
    faceless_shrine: "faceless shrine",
    fetid_mausoleum: "fetid mausoleum",
    ghoa_ruins: "ghoa ruins",
    haunted_refuge: "haunted refuge",
    heathens_keep: "heathens keep",
    iron_hold: "iron hold",
    leviathans_maw: "leviathans maw",
    lights_refuge: "lights refuge",
    lost_keep: "lost keep",
    maugans_works: "maugans works",
    oblivion: "oblivion",
    ruins_of_eridu: "ruins of eridu",
    serpents_lair: "serpents lair",
    shadowed_plunge: "shadowed plunge",
    steadfast_barracks: "steadfast barracks",
    witchwater: "witchwater",
    abandoned_mineworks: "abandoned mineworks",
    collapsed_vault: "collapsed vault",
    conclave: "conclave",
    corrupted_grotto: "corrupted grotto",
    crumbling_hekma: "crumbling hekma",
    crusaders_cathedral: "crusaders cathedral",
    deserted_underpass: "deserted underpass",
    fading_echo: "fading echo",
    forgotten_ruins: "forgotten ruins",
    hakans_refuge: "hakans refuge",
    halls_of_the_damned: "halls of the damned",
    heretics_asylum: "heretics asylum",
    inferno: "inferno",
    prison_of_caldeum: "prison of caldeum",
    putrid_aquifer: "putrid aquifer",
    renegades_retreat: "renegades retreat",
    sepulcher_of_the_forsworn: "sepulcher of the forsworn",
    shivta_ruins: "shivta ruins",
    sirocco_caverns: "sirocco caverns",
    sunken_library: "sunken library",
    tomb_of_the_saints: "tomb of the saints",
    uldurs_cave: "uldurs cave",
    yshari_sanctum: "yshari sanctum",
    tomb_of_hallows: "tomb of hallows",
    aldurwood: "aldurwood",
    broken_bulwark: "broken bulwark",
    calibels_mine: "calibels mine",
    demons_wake: "demons wake",
    domhainne_tunnels: "domhainne tunnels",
    ferals_den: "ferals den",
    flooded_depths: "flooded depths",
    garan_hold: "garan hold",
    hive: "hive",
    howling_warren: "howling warren",
    jalals_vigil: "jalals vigil",
    lubans_rest: "lubans rest",
    maddux_watch: "maddux watch",
    mariners_refuge: "mariners refuge",
    oldstones: "oldstones",
    penitent_cairns: "penitent cairns",
    raethwind_wilds: "raethwind wilds",
    sarats_lair: "sarats lair",
    stockades: "stockades",
    sunken_ruins: "sunken ruins",
    twisted_hollow: "twisted hollow",
    underroot: "underroot",
    vault_of_the_forsaken: "vault of the forsaken",
    whispering_pines: "whispering pines",
    wretched_delve: "wretched delve",
    ancients_lament: "ancients lament",
    betrayers_row: "betrayers row",
    bloodsoaked_crag: "bloodsoaked crag",
    buried_halls: "buried halls",
    carrion_fields: "carrion fields",
    champions_demise: "champions demise",
    charnel_house: "charnel house",
    dark_ravine: "dark ravine",
    forgotten_depths: "forgotten depths",
    grinning_labyrinth: "grinning labyrinth",
    guulrahn_canals: "guulrahn canals",
    guulrahn_slums: "guulrahn slums",
    komdor_temple: "komdor temple",
    mournfield: "mournfield",
    onyx_hold: "onyx hold",
    pallid_delve: "pallid delve",
    path_of_the_blind: "path of the blind",
    sealed_archives: "sealed archives",
    seaside_descent: "seaside descent",
    shifting_city: "shifting city",
    whispering_vault: "whispering vault",
  },
  minor: {
    monster_crowd_control_resist:
      "monster crowd control resist crowd control duration vs monsters is reduced by .",
    monster_critical_resist:
      "monster critical resist monster attacks reduce the damage of your critical strikes for seconds by , stacking up to .",
    monster_overpower_resist:
      "monster overpower resist monster attacks reduce the damage of your next overpower attack by . stacking up to .",
    monster_vulnerable_resist:
      "monster vulnerable resist duration of vulnerable effects vs monsters is reduced by .",
    backstabbers:
      "backstabbers close monster attacks from behind cause you to become vulnerable.",
    monster_attack_speed: "monster attack speed monsters attack faster.",
    monster_barrier:
      "monster barrier monsters gain of their maximum life as a barrier.",
    barrier_breakers: "barrier breakers monsters deal more damage to barriers.",
    slowing_projectiles:
      "slowing projectiles monster attacks from a distance have a chance to slow targets.",
    resource_burn:
      "resource burn after you spend resource, you burn an additional of the resource spent for every time you were hit by a distant enemy.",
    monster_bleeding_damage:
      "monster bleeding damage monsters deal an additional of their physical damage dealt as bleeding over seconds.",
    monster_burning_damage:
      "monster burning damage monsters deal an additional of their physical damage dealt as burning over seconds.",
    monster_cold_damage:
      "monster cold damage monsters deal an additional of their physical damage dealt as cold.",
    monster_shadow_damage_over_time:
      "monster shadow damage over time monsters deal an additional of their physical damage dealt as shadow over seconds.",
    monster_fire_damage:
      "monster fire damage monsters deal an additional of their physical damage dealt as fire.",
    monster_lightning_damage:
      "monster lightning damage monsters deal an additional of their physical damage dealt as lightning.",
    monster_poison_damage:
      "monster poison damage monsters deal an additional of their physical damage dealt as poison over seconds.",
    monster_shadow_damage:
      "monster shadow damage monsters deal an additional of their physical damage dealt as shadow.",
    monster_life: "monster life monsters gain extra life.",
    monster_regen:
      "monster regen nonboss monsters regen maximum life per second.",
    monster_bleed_resist:
      "monster bleed resist monsters take less bleeding damage.",
    monster_burning_resist:
      "monster burning resist monsters take less burning damage.",
    monster_cold_resist: "monster cold resist monsters take less cold damage.",
    monster_shadow_damage_over_time_resist:
      "monster shadow damage over time resist monsters take reduced damage from your shadow damage over time effects.",
    monster_fire_resist: "monster fire resist monsters take less fire damage.",
    monsters_lightning_resist:
      "monsters lightning resist monsters take less lightning damage.",
    melee_defenders:
      "melee defenders monsters take less damage from close targets",
    monster_physical_resist:
      "monster physical resist monsters take less physical damage.",
    monster_poison_resist:
      "monster poison resist monsters take less poison damage.",
    ranged_defenders:
      "ranged defenders monsters take less damage from distant targets.",
    monster_shadow_resist:
      "monster shadow resist monsters take less shadow damage.",
    monster_life_steal: "monster life steal nonboss monsters gain life steal.",
    dodge_breakers:
      "dodge breakers monster attacks from a distance reduce dodge chance by for seconds, stacking up to .",
    potion_breakers:
      "potion breakers monster attacks from a distance increase the cooldown of your next potion by seconds, up to seconds.",
    resistance_breakers:
      "resistance breakers monster attacks from a distance reduce resistance to all elements by for seconds, stacking up to .",
    monster_thorns: "monster thorns monsters reflect of damage.",
    armor_breakers:
      "armor breakers monster attacks from a distance reduce your armor by for seconds, stacking up to .",
    fire_traps: "fire traps additional fire traps appear in this dungeon.",
    unstoppable_monsters:
      "unstoppable monsters monsters become unstoppable when life drops below .",
  },
  major: {
    avenger:
      "avenger killing a monster enrages monsters near it after a short delay, making them deal more damage.",
    blood_blister:
      "blood blister killing a monster has a chance to spawn a blood blister. after a short time, it explodes, dealing heavy area damage.",
    death_pulse:
      "death pulse killing a monster releases a deadly shadow pulse after a short delay.",
    drifting_shade:
      "drifting shade drifting shade chases players. upon reaching them, it explodes for heavy damage and creates a nightmare field that dazes victims.",
    elemental_totems:
      "elemental totems elemental totems occasionally spawn in combat that buff close enemies after seconds if not destroyed.",
    empowered_elites_shock_lance:
      'empowered elites shock lance elites always have the "electric lance" affix.',
    empowered_elites_cold_enchanted:
      'empowered elites cold enchanted elites always have the "cold enchanted" affix.',
    empowered_elites_poison_enchanted:
      'empowered elites poison enchanted elites always have the "poison enchanted" affix.',
    empowered_elites_shadow_enchanted:
      'empowered elites shadow enchanted elites always have the "shadow enchanted" affix.',
    empowered_elites_suppressor:
      'empowered elites suppressor elites always have the "suppressor" affix.',
    empowered_elites_teleporter:
      'empowered elites teleporter elites always have the "teleporter" affix.',
    lightning_storm:
      "lightning storm lightning gathers above the player. get into the protection dome to avoid severe outcomes.",
    nightmare_portal:
      "nightmare portal while in combat, nightmare portals open randomly near players, pouring out dangerous monsters.",
    stormbanes_wrath:
      "stormbanes wrath stormbanes wrath chases players around, releasing deadly pulses when reaching a player.",
    volcanic:
      "volcanic while in combat, gouts of flame periodically erupt near players.",
  },
  positive: {
    quick_killer:
      "quick killer killing a monster grants attack and move speed. stacking up to .",
    reduce_cooldowns_on_kill:
      "reduce cooldowns on kill killing a monster reduces your cooldowns by . seconds.",
    battle_hardened:
      "battle hardened gain damage reduction for every health you are missing.",
    poisonous_evade:
      "poisonous evade using evade leaves a damaging pool of poison under the first enemy you evade through or at your destination.",
    nudging_evade: "nudging evade using evade pushes enemies back.",
    extra_shrines: "extra shrines extra shrines will appear in the dungeon.",
    fire_damage: "fire damage you deal more fire damage.",
    frost_damage: "frost damage you deal more frost damage.",
    gold_find: "gold find you find more gold.",
    increased_critical_strike:
      "increased critical strike your critical strike chance is increased by .",
    increased_healing:
      "increased healing your healing received is increased by .",
    lightning_damage: "lightning damage you deal more lightning damage.",
    magic_find: "magic find you find more items from enemies.",
    lightning_caller:
      "lightning caller you occasionally call down lightning strikes that damage nearby enemies.",
    physical_damage: "physical damage you deal more physical damage.",
    poison_damage: "poison damage you deal more poison damage",
    shadow_damage: "shadow damage you deal more shadow damage.",
    control_impaired_explosions:
      "control impaired explosions being hit by control impairing effects creates an explosion around you.",
    thorns:
      "thorns after attacking an enemy, your thorns are increased by for seconds, up to at max stacks.",
    ancestors_favour:
      "extra lunar shrines and more glyph experience earned at the end of this dungeon.",
  },
};
